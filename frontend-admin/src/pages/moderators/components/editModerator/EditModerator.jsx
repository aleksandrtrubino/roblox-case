...
import {useCreateUserMutation, useDeleteUserByIdMutation, usePatchUserByIdMutation} from "../../../../api/userApi"; // Замена на мутацию для обновления профиля

const NAME_REGEX = /^[a-zA-Z0-9]+$/;
const PASSWORD_REGEX = /^(?=.*[a-z])(?=.*[0-9]).{8,24}$/;
const EMAIL_REGEX = /^[\w-.]+@([\w-]+\.)+[\w-]{2,4}$/;

export const EditModerator = ({ onClose, moderator }) => {

    const [nickname, setNickname] = useState(moderator.nickname);
    ...
    const [contactLink, setContactLink] = useState(moderator.contact.link);

    const [contactTypeId, setContactTypeId] = useState(moderator.contact.type.id);

    useEffect(() => {
        if(contactTypeId === moderator.contact.type.id){
            setContactLink(moderator.contact.link)
        }
    }, [contactTypeId]);
    ...
    const [editModerator] = usePatchUserByIdMutation();
    const [deleteModerator] = useDeleteUserByIdMutation();
    ...
    const handleSubmit = async (e) => {
        ...
        if (validFields()) {
            try {
                ...
                const response = await editModerator({userId: moderator.id, userDto: profileData}).unwrap();
                onClose()
            } catch (error) {
                console.log(error);
            }
    ...
    const handleDelete = async() =>{
        try{
            await deleteModerator(moderator.id);
            onClose()
        }
        catch{
                ...
                <section className={styles.section}>
                    <form className={styles.form} onSubmit={handleSubmit}>
                        ...
                        <div className={styles.buttonsWrapper}>
                            <button className={styles.deleteButton} type='button' onClick={()=>handleDelete()}>Удалить</button>
