...
import styles from "./CreateModerator.module.scss";
import { useNavigate } from "react-router-dom";
import {useRegisterMutation} from "../../../../api/authApi";
import {useCreateUserMutation} from "../../../../api/userApi"; // Замена на мутацию для обновления профиля

const NAME_REGEX = /^[a-zA-Z0-9]+$/;
const PASSWORD_REGEX = /^(?=.*[a-z])(?=.*[0-9]).{8,24}$/;
const EMAIL_REGEX = /^[\w-.]+@([\w-]+\.)+[\w-]{2,4}$/;

export const CreateModerator = ({ onClose }) => {
    ...
    const [createModerator] = useCreateUserMutation();
    ...
    const handleSubmit = async (e) => {
        ...
        if (validFields()) {
            try {
                ...
                const response = await createModerator(profileData).unwrap();
                onClose()
            } catch (error) {
                console.log(error);
            }
